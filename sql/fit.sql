select accountid                              account_id,
       territory,
       closedate::DATE - INTERVAL '90 day' as relevant_date,
       case
           when new_product not in ('JFrog Pro', 'JFrog Pro - New Pricing') then 1
           else 0 end                      as class
into #base_accounts
from (
         ------- base population
------ take won and closed from the opp table
         (select distinct accountid,
          ...
          where type != 'New Business'
            and iswon = TRUE
            and isclosed = TRUE
            and closedate >= '2018-01-01') as opps
             join
             (select product_id, product_name old_product
              from dims.dim_products ---- add previous product name
              where product_name in
                    ('JFrog Pro - New Pricing', 'JFrog Pro')) as old_products
             on opps.original_product__c = old_products.old_product
             join
             (select product_id, product_name new_product
              from dims.dim_products ---- add renewal product name
              where product_family = 'Artifactory'
                and type = 'License'
                and product_team != 'None'
                and product_name not in ('Support Services for JFrog Pro Edition')
                and lower(product_name) not like '%cloud%') as new_products
