...
SELECT account_id,
       ...
       sum(CASE
               WHEN jira_case_resolution_status IN
                    ('Wont Fix', 'Not Fixed', 'Will not implement', 'Unresolved',
                     'Wont Do') THEN 1
               ELSE 0
...
INTO #repositories
FROM (SELECT environment_service_id,
             repo.account_id,
             relevant_date,
             CLASS,
             CASE
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date),
                         -1) AND date_trunc('day', b.relevant_date)
                     THEN '3 Months' -- Actually 1 month from relevant date
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -4) AND add_months(
                         date_trunc('day', b.relevant_date), -3)
                     THEN '4 Months' -- Actually 1 quarter from relevant date
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -7) AND add_months(
                         date_trunc('day', b.relevant_date), -6)
                     THEN '5 Months' -- Actually 2 quarters from relevant date
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -2) AND add_months(
                         date_trunc('day', b.relevant_date), -1)
                     THEN '6 Months' -- Actually 2 months from relevant date
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -3) AND add_months(
                         date_trunc('day', b.relevant_date), -2)
                     THEN '7 Months' -- Actually 3 months from relevant date
                 END  AS period_range,
             max(CASE
...
FROM (SELECT a.account_id,
             ...
             CASE
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date),
                         -1) AND date_trunc('day', b.relevant_date) THEN '3 Months'
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -4) AND add_months(
                         date_trunc('day', b.relevant_date), -3) THEN '4 Months'
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -7) AND add_months(
                         date_trunc('day', b.relevant_date), -6) THEN '5 Months'
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -2) AND add_months(
                         date_trunc('day', b.relevant_date), -1) THEN '6 Months'
                 WHEN date_trunc('day', art_create_date) BETWEEN add_months(
                         date_trunc('day', b.relevant_date), -3) AND add_months(
                         date_trunc('day', b.relevant_date), -2) THEN '7 Months'
                 END                  AS period_range,
...
SELECT a.account_id,
       a.relevant_date,
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_artifacts_count_change
INTO #days_from_artifacts_count
...
SELECT a.account_id,
       ...
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_binaries_count_change
INTO #days_from_binaries_count
FROM #storage_over_time AS a
         JOIN
     (SELECT account_id,
             relevant_date,
...
SELECT a.account_id,
       ...
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_artifacts_size_change
INTO #days_from_artifacts_size
FROM #storage_over_time AS a
         JOIN
     (SELECT account_id,
             relevant_date,
...
SELECT a.account_id,
       ...
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_binaries_size_change
INTO #days_from_binaries_size
FROM #storage_over_time AS a
         JOIN
     (SELECT account_id,
             relevant_date,
...
SELECT a.account_id,
       ...
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_items_count_change
INTO #days_from_items_count
FROM #storage_over_time AS a
         JOIN
     (SELECT account_id,
             relevant_date,
...
SELECT a.account_id,
       a.relevant_date,
       a.class,
       datediff('day', max(art_create_date), a.relevant_date) AS days_from_users_change
INTO #days_from_users
...
SELECT a.account_id,
       ...
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_permissions_change
INTO #days_from_permissions
FROM #users_over_time AS a
         JOIN
     (SELECT account_id,
             relevant_date,
...
SELECT a.account_id,
       ...
       a.class,
       datediff('day', max(art_create_date),
                a.relevant_date) AS days_from_internal_groups_change
INTO #days_from_internal_groups
FROM #users_over_time AS a
         JOIN
     (SELECT account_id,
             relevant_date,
...
SELECT a.account_id,
       relevant_date,
       count(DISTINCT CASE
                          WHEN movement_date BETWEEN dateadd(MONTH, -12, a.relevant_date) AND a.relevant_date
                              THEN cases.case_id
           END) AS cases_within_last_year,
       count(DISTINCT CASE
                          WHEN movement_date BETWEEN dateadd(MONTH, -3, a.relevant_date) AND a.relevant_date
                              THEN cases.case_id
           END) AS cases_within_3_last_months
INTO #support
FROM qoc.stg_events_measures AS EVENTS
         LEFT JOIN salesforce_repo.fact_cases AS cases
                   ON LEFT(cases.case_id, 15) = LEFT(events.case_id, 15)
                       AND events.parameter_id IN (10)
         INNER JOIN #base_accounts AS a
                    ON LEFT(cases.account_id, 15) = LEFT(a.account_id, 15)
...
INTO #poor_cases
FROM #base_accounts AS a
         JOIN data_science.satisfaction_case_level AS b
              ON LEFT(a.account_id, 15) = LEFT(b.account_id, 15)
...
SELECT a.account_id,
       ...
       avg(CASE
               WHEN status IN ('Closed', 'Resolved') THEN datediff('day', createddate,
                                                                   coalesce(resolved_time__c, closeddate))
           END) AS avg_resolution_days
INTO #cases_agg
FROM #base_accounts AS a
         LEFT JOIN salesforce_repo.case AS b
                   ON LEFT(a.account_id, 15) = LEFT(b.accountid, 15)
...
FROM facts.fact_technical_sessions s
         INNER JOIN #base_accounts AS a
                    ON LEFT(s.account_id, 15) = LEFT(a.account_id, 15)
...
FROM (SELECT *
      ...
      WHERE recordtypeid = '012w0000000R1Yp'
        AND stage__c != 'Unprovided') AS st
         RIGHT JOIN #base_accounts AS ar
                    ON LEFT(st.account__c, 15) = LEFT(ar.account_id, 15)
...
INTO #trials
FROM #base_accounts AS a
         LEFT JOIN salesforce_repo.trial__c AS b
                   ON LEFT(a.account_id, 15) = LEFT(b.account__c, 15)
WHERE createddate BETWEEN add_months(date_trunc('month', a.relevant_date),
...
FROM salesforce_repo.contact AS c
         RIGHT JOIN #base_accounts AS ar
                    ON LEFT(c.accountid, 15) = LEFT(ar.account_id, 15)
...
INTO #contracts
FROM #base_accounts AS a
         LEFT JOIN salesforce_repo.contract AS c
                   ON LEFT(c.accountid, 15) = LEFT(a.account_id, 15)
...
FROM salesforce_repo.qoe_scores AS d
         RIGHT JOIN #base_accounts AS ar
                    ON LEFT(d.account_id, 15) = LEFT(ar.account_id, 15)
...
select da.account_id,
       ...
       dozisf__contact__c,
       case
           when lower(dozisf__job_title__c) like '%engineer%' or
                lower(dozisf__job_title__c) like '%developer%' or
                lower(dozisf__job_title__c) like '%software%'
               then 1
           else 0 end              as is_developer,
       case
           when lower(dozisf__job_title__c) like '%devops%' then 1
           else 0 end              as is_devops_engineer,
       case
...
from salesforce_repo.dozisf__zoominfo__c as zi
         join dims.dim_contacts dc
              on LEFT(zi.dozisf__contact__c, 15) = LEFT(dc.contact_id, 15)
...
select roles.*,
       ...
       founded_year
into #zoom_info_agg
from (SELECT account_id, max(total_employees_range) as total_employees_range
      FROM (SELECT account_id,
                   total_employees_range,
     ...
     (select account_id,
             ...
             count(distinct case
                                when is_developer = 1
                                    then dozisf__contact__c end) as developers,
             count(distinct case
                                when is_devops_engineer = 1
                                    then dozisf__contact__c end) as devops_engineers,
             count(distinct case
                                when is_engineer = 1
...
SELECT account_id,
       ...
       max(knowledge_views)   AS knowledge_views
INTO #key_pages_views
FROM (SELECT a.account_id,
             b.contact_id,
             a.relevant_date,
             sum(CASE
      ...
      FROM #base_accounts AS a
               LEFT JOIN dims.dim_contacts AS b
                         ON LEFT(a.account_id, 15) = LEFT(b.account_id, 15)
...
INTO #cloud_sub
FROM #base_accounts AS a
         JOIN salesforce_repo.contact AS b
              ON LEFT(a.account_id, 15) = LEFT(b.accountid, 15)
...
select ar.account_id,
       ...
       count(distinct (case
                           when term in
                               ...
                               then instance_date::date end)) as n_ha_mentioned_cases,
       count(distinct (case
                           when term in ('balancer', 'balancing', 'balance')
                               then instance_date::date end)) as n_bal_mentioned_cases,
       count(distinct (case
                           when term = 'enterprise'
                               then instance_date::date end)) as n_ent_mentioned_cases,
       count(distinct (case
                           when term = 'multiple'
                               then instance_date::date end)) as n_mul_mentioned_cases,
       count(distinct (case
                           when term = 'replications'
                               then instance_date::date end)) as n_rep_mentioned_cases,
       count(distinct (case
                           when term in
                                ('disaster recovery', 'downtime', ' dr', 'down time',
                                 'bad performance')
                               then instance_date::date end)) as n_bad_mentioned_cases
into #triggers_cases
...
select ar.account_id,
       ...
       count(distinct (CASE
                           WHEN term IN
                                ('disaster recovery', 'downtime', 'bad performance')
                               THEN instance_date
...
INTO #emails
FROM #base_accounts AS a
         JOIN salesforce_repo.task AS b
              ON LEFT(a.account_id, 15) = LEFT(b.accountid, 15)
...
SELECT account_id,
       ...
       datediff('day', max(CASE
                               WHEN salesloft1__salesloft_type__c = 'Reply'
                                   THEN createddate
...
SELECT DISTINCT a.account_id,
                ...
                case
                    when cb.founded_year is null or cb.founded_year = '' then -1
                    else
                        extract(YEAR
                                FROM a.relevant_date) -
                        cb.founded_year end                    AS company_age,
