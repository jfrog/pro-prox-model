...
select distinct account_id, territory, 1 as class,
      ADD_MONTHS(DATE_TRUNC('month',min(event_month)),-3) as relevant_date
into #upsell_accounts
from #contracts_accounts_tmp
where prev_product_name = 'JFrog Pro'
and new_product_name in ('JFrog Pro X','JFrog Pro Plus')
and class = 1
group by 1,2,3;
--------------------
-- not Upsells
--------------------
drop table if exists #not_upsell_accounts;
select distinct a.account_id, a.territory, 0 as class
into #not_upsell_accounts
from #contracts_accounts_tmp as a
left join #upsell_accounts  as b
on a.account_id = b.account_id
where prev_product_name = 'JFrog Pro'
and new_product_name = 'JFrog Pro'
and b.account_id is null;
---------------------------------
-- random_relevant_month
---------------------------------
drop table if exists #random_relevant_month;
select account_id, to_Date(extract(MONTH from relevant_date)::varchar ||' '|| extract(YEAR from relevant_date)::varchar, 'mm YYYY') as relevant_date
...
into #base_accounts
from
(select distinct a.account_id, a.territory, class, r.relevant_date as relevant_date
from #not_upsell_accounts as a
left join #random_relevant_month as r on r.account_id = a.account_id)
union all (select * from #upsell_accounts);
