...
-- base population
--------------------
DROP TABLE IF EXISTS #base_accounts2;

SELECT a.*,
       territory__c AS territory INTO #base_accounts2
FROM (
        (SELECT accountid AS account_id,
                0 AS CLASS,
                add_months(date_trunc('month', closedate), -3)::date AS relevant_date
         FROM salesforce_repo.opportunity
         WHERE product_type__c = 'JFrog Pro'
           AND original_product__c = 'JFrog Pro'
           AND extract(YEAR
                       FROM createddate) >= 2018 --and (lower(type) like '%renewal%' or lower(type) like '%upsell%')
--and lower(type) not like '%downsell%'
AND iswon = 'true')
      UNION
        (SELECT accountid AS account_id,
                1 AS CLASS,
                add_months(date_trunc('month', closedate), -3)::date AS relevant_date
         FROM salesforce_repo.opportunity
         WHERE product_type__c = 'JFrog Pro X'
           AND original_product__c = 'JFrog Pro'
           AND extract(YEAR
                       FROM createddate) >= 2018
           AND lower(TYPE) not like '%downsell%'
           AND iswon = 'true')) AS a
JOIN salesforce_repo.account AS c ON a.account_id = left(c.accountid_full, LEN (c.accountid_full) -3)
WHERE relevant_date IS NOT NULL;


DROP TABLE IF EXISTS #base_accounts;
-- This is to remove weird cases when an account with a specific relevant date has 2 contradicting transfers
SELECT a.* INTO #base_accounts
FROM #base_accounts2 AS a
JOIN
  (SELECT account_id
   FROM
     (SELECT account_id,
   ...
   HAVING max(CASE
                  WHEN COUNT > 1 THEN 1
                  ELSE 0
              END) = 0) AS b ON a.account_id = b.account_id;

/*drop table if exists #months_from_renwal;
select account_id, relevant_date, class,
      datediff('month', max(closedate), relevant_date) as months_from_renewal
into #months_from_renwal
from #base_accounts as a
join salesforce_repo.opportunity as b
on a.account_id = b.accountid
where closedate <= relevant_date
group by 1,2,3;*/
...
SELECT a.account_id,
       ...
       count(distinct(CASE
                      ...
                      END)) AS count_ent,
       count(distinct(CASE
                          WHEN contract_value__c = 'JFrog Pro X' THEN startdate
                      END)) AS count_prox,
       count(distinct(CASE
                          WHEN contract_value__c = 'JFrog Pro' THEN startdate
