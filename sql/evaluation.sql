...
from (select scores.*,
             cast(date_trunc('month', ops.createddate) as date) as month,
             case
                 when contract_type__c = '' then type
                 else contract_type__c end                      as status,
             case
                 when territory__c = 'Americas' then 'Americas'
                 else 'EMEA/APAC' end                           as territory,
             case
                 when (status = 'JFrog Pro X' or product_type__c = 'JFrog Pro X')
                     then 1.000
                 else 0.000 end                                 as upsell
      from ((select a.account_id,
                    b.rating,
                    min(insert_date) as first_scoring_date
             from (select account_id,
                          case
                              when (max(prob) > 0.5 and max(prob) < 0.625) then 'medium'
